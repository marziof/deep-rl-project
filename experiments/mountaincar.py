...
def run_experiment_mountaincar_PPO(env_vector, agent, logger, n_iterations=100, eval_interval=10, seed=0, create_videos=False, video_interval=500):
    #rewards = []
        ...
        if it % eval_interval == 0:
            ...
            for s in range(3):
                if create_videos and s == 0 and (it%video_interval == 0): 
                    eval_env = make_env_render("MountainCar-v0", seed=seed + 1000 + s)
                    scores.append(evaluate_PPO(eval_env, agent, n_episodes=5, visualize=True, video_title=f"mountaincar_ppo_seed_{seed}_episode_{it}"))
                    eval_env.close()
                else:
...
def run_experiments_mountaincar_PPO(agent_fn, seeds, n_episodes=100, eval_interval=10, create_videos=False, video_interval=500):
    all_logs = []

    for seed in seeds:

        
        env_test = gym.make("MountainCar-v0") # we will create vectorized envs later, here we just need it to get the state and action space dimensions for the agent initialization
        state_dim = env_test.observation_space.shape[0]
        action_space = env_test.action_space

        agent = agent_fn(action_space, state_dim)
        envs  = gym.vector.SyncVectorEnv([
            lambda: gym.make("MountainCar-v0") for _ in range(agent.n_actors)
        ])
        logger = Logger()

        try:
            logger = run_experiment_mountaincar_PPO(envs, agent, logger, n_iterations=n_episodes, eval_interval=eval_interval, seed=seed, create_videos=create_videos, video_interval=video_interval)
        finally:
            envs.close()
            env_test.close()
