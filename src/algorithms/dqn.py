...
class DQNAgent:
    ...
    def __init__(self, action_space, state_dim, hidden_units, gamma, batch_size, eps, eps_min, eps_decay, target_update_freq, buffer_capacity, lr):
        #super().__init__()
        ...
        self.q_net = MLP(input_dim=state_dim, output_dim=action_space.n, hidden_units = self.hidden_dim) # input_dim = dim of state space, output_dim = dim of action space
        self.target_net = MLP(input_dim=state_dim, output_dim=action_space.n, hidden_units = self.hidden_dim)
