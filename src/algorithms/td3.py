...
from src.networks.mlp import Actor
...
class TD3Agent:
    ...
    def __init__(self, action_space, state_dim, gamma, batch_size, policy_delay, buffer_capacity, lr, sigma, sigma_tilde, c, tau):
        ...
        self.crit_net1 = MLP(input_dim=state_dim + action_dim, output_dim=1, hidden_units=256)
        ...
        self.target_net1.load_state_dict(self.crit_net1.state_dict())
        self.target_net2.load_state_dict(self.crit_net2.state_dict())
        ...
        # Cache action bounds as numpy to avoid repeated tensor conversions
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            # Pre-compute concatenations once, reuse for both target networks
            next_state_action = torch.cat([next_states_tensor, a_tilde], dim=1)
            q1_target = self.target_net1(next_state_action)
            q2_target = self.target_net2(next_state_action)
        ...
        q1_values = self.crit_net1(state_action)
        q2_values = self.crit_net2(state_action)
