...
for time_per_actor in times_per_actor:
    ...
    ppo_logs = run_experiments_PPO(
        env_name=env_name,
        agent_fn=lambda action_space, state_dim: PPOAgentContinuous(
        ...
        create_videos=False,
        video_interval=100 #MUST BE DIVISIBLE BY eval_interval, otherwise we will never save videos
    )
    ...
    plot_learning_curve(eval_episodes, mean_eval_rewards_ppo, std_eval_rewards_ppo, title=f"{env_name} PPO (time_per_actor={time_per_actor})", save_path=f"media/plots/{env_name}_ppo_time{time_per_actor}_20_05")
